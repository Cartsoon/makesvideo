...
---
Generated by IDENGINE VIDEO FACTORY
`;
  ...
  app.get("/api/ai/status", async (req, res) => {
    try {
      const { getApiProviderConfig, getAvailableProviders, getProviderDisplayName } = await import("./ai/provider");
      ...
      const providerConfig = await getApiProviderConfig(storage);
      const availableProviders = await getAvailableProviders(storage);
      const activeProvider = availableProviders.find(p => p.type === providerConfig.type);
      
      res.json({
        available: activeProvider?.available && !fallbackMode,
        providerType: providerConfig.type,
        providerName: getProviderDisplayName(providerConfig.type, "ru"),
        providerNameEn: getProviderDisplayName(providerConfig.type, "en"),
        verified: providerConfig.verified,
        lastVerified: providerConfig.lastVerified,
        fallbackMode,
  ...
  app.post("/api/ai/verify", async (req, res) => {
    try {
      ...
      const { verifyApiConnection, resetCustomApiCache } = await import("./ai/provider");
      
      const result = await verifyApiConnection(providerType || "default");
      
      if (result.success) {
        await storage.setSetting("apiVerified", "true");
        await storage.setSetting("apiLastVerified", new Date().toISOString());
        resetCustomApiCache();
      }
      
      res.json(result);
    } catch (error: any) {
      res.json({ success: false, error: error.message || "Verification failed" });
    }
  });
  ...
  app.post("/api/ai/provider", async (req, res) => {
    try {
      const { providerType } = req.body;
      const { resetCustomApiCache, getAvailableProviders } = await import("./ai/provider");
      
      const available = await getAvailableProviders(storage);
      const selected = available.find(p => p.type === providerType);
      ...
      if (!selected.available) {
        return res.status(400).json({ error: selected.reason || "Provider not available" });
      }
      
      await storage.setSetting("apiProviderType", providerType);
      await storage.setSetting("apiVerified", "false");
      resetCustomApiCache();
      
      const { refreshLLMProvider } = await import("./providers");
      await refreshLLMProvider();
      
      res.json({ success: true, providerType });
    } catch (error: any) {
      res.status(500).json({ error: error.message || "Failed to switch provider" });
    }
  });

  // Save provider credentials (API key + base URL)
  app.post("/api/ai/credentials", async (req, res) => {
    try {
      ...
      const keyName = `${providerType}_api_key`;
      const urlName = `${providerType}_base_url`;
      
      await storage.setSetting(keyName, apiKey);
      if (baseUrl) {
        await storage.setSetting(urlName, baseUrl);
      }
      
      // Verify the connection
      const { verifyApiConnectionWithCredentials, resetCustomApiCache } = await import("./ai/provider");
      const result = await verifyApiConnectionWithCredentials(providerType, apiKey, baseUrl);
  ...
  app.post("/api/settings", async (req, res) => {
    try {
      ...
      // General settings only - AI is now unified via Replit AI Integrations
      ...
      if (useCustomApi !== undefined) {
        await storage.setSetting("useCustomApi", String(useCustomApi));
        // Reset the cached setting so next AI call uses new value
        const { resetCustomApiCache } = await import("./ai/provider");
        resetCustomApiCache();
      }
  ...
  app.post("/api/generate/text-to-video", async (req, res) => {
    try {
      ...
      if (!hasOpenAI || fallbackMode) {
        const errorMsg = language === "ru" 
          ? "AI генерация недоступна. Проверьте OPENAI_API_KEY."
          : "AI generation unavailable. Check OPENAI_API_KEY.";
