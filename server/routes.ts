...
`;
  ...
  app.get("/api/ai/status", async (req, res) => {
    try {
      ...
      const hasCustomKey = !!process.env.CUSTOM_OPENAI_API_KEY;
      ...
      if (useCustomApi && hasCustomKey) {
        provider = "custom_openai";
        available = !fallbackMode;
      } else if (hasOpenAI) {
      ...
      res.json({
        ...
        customApiAvailable: hasCustomKey,
        useCustomApi
      });
    } catch (error) {
    ...
    try {
      ...
      if (!hasOpenAI || fallbackMode) {
        const errorMsg = language === "ru" 
          ? "AI генерация недоступна. Проверьте подключение Replit AI Integrations."
          : "AI generation unavailable. Check Replit AI Integrations connection.";
