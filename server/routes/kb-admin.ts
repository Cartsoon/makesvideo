...
router.post("/index/generateChunks", async (req: Request, res: Response) => {
  try {
    ...
    const prompt = `Ты эксперт по созданию базы знаний для RAG-системы видеоконтента.

ЗАДАЧА: Извлеки ${chunkCount} ключевых инсайтов из текста.

ПРАВИЛА:
1. Каждый чанк - отдельный инсайт или совет
2. Краткость: 1-3 предложения на инсайт
3. Формулируй как конкретный практический совет
4. Не повторяй одинаковые идеи
5. Выбирай самые ценные и применимые инсайты

ОБЯЗАТЕЛЬНЫЙ ФОРМАТ КАЖДОГО ЧАНКА:
ТИП: [категория - например: Сторителлинг, Доверие, Формат эксперта, Сценарный приём, Механика платформ, Хуки, Монтаж, Музыка, Голос, Тренды и т.д.]
КОНТЕКСТ: [где применяется - например: Личный контент, Истории, Концовка видео, Reels и Shorts, Начало видео, Личный бренд и т.д.]

[Краткий инсайт в 1-3 предложения]

ПРИМЕРЫ ХОРОШИХ ЧАНКОВ:

ТИП: Доверие
КОНТЕКСТ: Личный контент

Честные истории о провалах усиливают доверие и вовлечённость аудитории.

---

ТИП: Механика платформ
КОНТЕКСТ: Reels и Shorts

Когда зритель пишет комментарий, видео продолжает крутиться — глубина просмотра растёт.

Для каждого чанка определи:
- level: critical/important/normal/supplementary (насколько важен инсайт)
...
2. Краткий и ёмкий - 1-3 предложения
3. Формулируй как конкретный совет или инсайт
4. НЕ дублируй информацию из существующих чанков
5. Выбери новую тему из документа

ОБЯЗАТЕЛЬНЫЙ ФОРМАТ ЧАНКА:
ТИП: [категория - например: Сторителлинг, Доверие, Формат эксперта, Сценарный приём, Механика платформ, Хуки, Монтаж, Музыка, Голос, Тренды и т.д.]
КОНТЕКСТ: [где применяется - например: Личный контент, Истории, Концовка видео, Reels и Shorts, Начало видео, Личный бренд и т.д.]

[Краткий инсайт или совет в 1-3 предложения]
